...
warp.alg <- function(Y.arg=NULL, params = NULL, delta = 1,  n.saem = 10, n.particles = 2000, n.mcmc = 50, n.memory = 5, n.siman = 5000, SimAn.MinPeaks = .5, SimAn.MaxPeaks = 12, SimAn.Pen = 1, Csyntax = FALSE, justPlot = FALSE){
  ...
  if(Csyntax){ # USE C-syntax (computational efficient)
    ...
    par_names <- c("bta", "a", "omega", "A", "B", "b", "sigma", "X.0", "gX.0")
...
")
  ...
  double f_x = A * sin(gX + b) - B * cos(2*(gX + b));
...
")
    ...
    dmeas_tw <- Csnippet("
  lik = dnorm(Y, A * sin(gX + b) - B * cos(2*(gX + b)), sigma, give_log);
") 
  }else{ # Use R syntax (less computationally efficient)
    
    par_names <- c("beta", "a", "omega", "A", "B", "b", "sigma", "X_0", "gX_0")
    ...
    rproc_tw <- function(X, gX, beta, a, omega, ..., delta.t) {
      ...
      # observation simulator (rmeasure)
      rmeas_tw <- function(X, gX, A, B, b, sigma, ...) {
        
        # Compute f(x)
        f_x <- A * sin(gX + b) + B * cos(2 * (gX + b) + pi)
      ...
      # measurement density (dmeasure)
      dmeas_tw <- function(A, B, b, sigma, X, gX, Y, ... , log = FALSE) {
  ...
  if(is.null(Y.arg) & is.null(params)){ # draw random parameters, simulate, then estimate
    ...
    if(Csyntax){
      theta <- c(bta = beta, omega = omega, a = a, A = A, 
                 B = B, b = b, sigma = sigma, X.0 = a, gX.0 = 0)
    }else{ # R syntax
      theta <- c(beta = beta, omega = omega, a = a, A = A, 
                 B = B, b = b, sigma = sigma, X_0 = a, gX_0 = 0)
    }
    ...
    if(Csyntax){
      theta <- c(bta = params[1], omega =  params[2], a =  params[3], A =  params[4], 
                 B =  params[5], b =  params[6], sigma =  params[7], X.0 = a, gX.0 = 0)
      N <- params[8]
    }else{ # R-syntax
      theta <- c(beta = params[1], omega =  params[2], a =  params[3], A =  params[4], 
                 B =  params[5], b =  params[6], sigma =  params[7], X_0 = 0, gX_0 = 0)
      N <- params[8]
    }
  ...
  print("Initialization step: Simulated Annealing.")
  simAn.data <- SimAn.Init(tw.pomp@data[1,], iterations=n.siman, minCycles = SimAn.MinPeaks, maxCycles = SimAn.MaxPeaks, maxRatio = 5, penaltyWeight = SimAn.Pen)
  ...
  beta0 <- 0.1
  omega0 <- sqrt(a0*0.1) # omega^2/beta =  a < 2a (feller) and beta = 0.1
  ...
  alpha[(n.squeeze+1):n.saem] <- ((n.squeeze+1):n.saem - n.squeeze)^(-1)
  ...
  while(m  < n.saem+1 && m_eff < 5*n.saem){
    ...
    if((m-1)%%m_update == 0) print(paste("SAEM:", m-1))
    ...
    print(paste("Constraints met:", all_constraints_met))
    ...
    if(all_constraints_met){ # only update "information" if constraints are met
      ...
      # Non-linear least squares estimation of A, B and b
      nls.obj <- nlsLM( 
        Y0 ~ f(gx0, B.c + dA.c, B.c, b.c),
        start = list(B.c = B0, dA.c = A0 - B0, b.c = b0 - floor(b0/(2*pi))*(2*pi)),
        lower = c(1/100*l, 1/100*l, 0),  # Lower bounds
        upper = c(l, l, 2*pi)   # Upper bounds 
      )
      
      A0 <- coef(nls.obj)[[1]] + coef(nls.obj)[[2]]
      B0 <- coef(nls.obj)[[1]]
      b0 <- coef(nls.obj)[[3]]
      b0 <- b0-floor(b0/(2*pi))*(2*pi)
      sigma0 <- sqrt(mean((Y0 - f(gx0, A0, B0, b0))^2))
  ...
  fit.data <- list(Y = Y0,
                   Y.init.hat = simAn.data$Yfit,
                   Y.hat = f(gx0, A0, B0, b0),
                   gx.hat = gx0,
                   xi.hat = xi0,
                   param.est = param.est,
