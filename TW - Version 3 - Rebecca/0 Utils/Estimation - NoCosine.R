...
###############################################
warp.alg <- function(Y.arg=NULL, params = NULL, delta = 1,  n.saem = 10, n.particles = 2000, n.mcmc = 50, n.memory = 5, MinPeaks = .5, MaxPeaks = 12, Csyntax = FALSE, justPlot = FALSE){
...
") 
  ...
  # --- Initialization using Heuristics --- #
  ...
  smc.fit <- smc.fit <- SMCplus(Y0, 
                                n_part = 10000,
                                c(A0, sigma0, b0),
                                a_range = c(2*pi*MinPeaks/(N*delta),2*pi*MaxPeaks/(N*delta)))
  a0 <-  smc.fit$smc$a_est
  beta0 <-  smc.fit$smc$beta_est
  omega0 <-  smc.fit$smc$omega_est
  ...
  while(m  < n.saem+1 && m_eff < 5*n.saem){
    ...
    if(all_constraints_met){ # only update "information" if constraints are met
      ...
      sigma0 <- sqrt(mean((Y0 - f(gx0, A0, b0))^2))
