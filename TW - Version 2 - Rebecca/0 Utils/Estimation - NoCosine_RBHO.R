...
############################################### HAR RETTET SimAn.MinPeaks FRA 0.5 TIL 10
warp.alg <- function(Y.arg=NULL, params = NULL, delta = 1,  n.saem = 10, n.particles = 2000, n.mcmc = 50, n.memory = 5, n.siman = 5000, SimAn.MinPeaks = .5, SimAn.MaxPeaks = 12, SimAn.Pen = 1, Csyntax = FALSE, justPlot = FALSE){
  ...
  if(Csyntax){ # USE C-syntax (computational efficient)
    ...
    par_names <- c("bta", "a", "omega", "b", "sigma", "X.0", "gX.0")
...
")
    ...
    dmeas_tw <- Csnippet("
  lik = dnorm(Y, 1 * sin(gX + b), sigma, give_log);
") 
  }else{ # Use R syntax (less computationally efficient)
    
    par_names <- c("beta", "a", "omega", "b", "sigma", "X_0", "gX_0")
    ...
    # observation simulator (rmeasure)
    rmeas_tw <- function(X, gX, b, sigma, ...) {
    ...
    # measurement density (dmeasure)
    dmeas_tw <- function(b, sigma, X, gX, Y, ... , log = FALSE) {
  ...
  if(is.null(Y.arg) & is.null(params)){ # draw random parameters, simulate, then estimate
    ...
    if(Csyntax){
      theta <- c(bta = beta, omega = omega, a = a, 
                 b = b, sigma = sigma, X.0 = a, gX.0 = 0)
    }else{ # R syntax
      theta <- c(beta = beta, omega = omega, a = a, 
    ...
    if(Csyntax){
      theta <- c(bta = params[1], omega =  params[2], a =  params[3], 
                 b =  params[4], sigma =  params[5], X.0 = a, gX.0 = 0)
      N <- params[6]
    }else{ # R-syntax
      theta <- c(beta = params[1], omega =  params[2], a =  params[3], 
                 b =  params[4], sigma =  params[5], X_0 = 0, gX_0 = 0)
      N <- params[6]
    }
  ...
  while(m  < n.saem+1 && m_eff < 5*n.saem){
    ...
    if(all_constraints_met){ # only update "information" if constraints are met
      ...
      nls.obj <- nlsLM( 
        Y0 ~ f(gx0, 1, 0, b.c),
        start = list(b.c = b0 - floor(b0/(2*pi))*(2*pi)),
        lower = c(0),  # Lower bounds
        upper = c(2*pi)   # Upper bounds 
      )
