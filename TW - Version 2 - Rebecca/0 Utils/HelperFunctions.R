...
tw.LogLik <- function(Y.obs, gx.arg, theta.arg, delta.arg, complete=FALSE){
  ...
  ll <- sum(dnorm(Y.obs, f(gx.arg,  A.arg, B.arg, b.arg), sigma.arg, log = TRUE))
...
f <- function(gx.arg, A.arg = A, B.arg = B, b.arg = b) {
  A.arg * sin(gx.arg + b.arg) +
    B.arg * cos(2 * (gx.arg + b.arg)+ pi)
}

# ------ END ------- #

# INITIALIZATION method return a list element containing 5 items:
# item 1: pars      list (a0, b0, A0, B0, sigma0, beta0, psi0, omega2.0)
...
#####################################################################################################
## INITIALIZATION METHOD: Fit model using Simulated Annealing and assume g(x) = piecewise linear ##
#####################################################################################################
SimAn.Init <- function(Y, iterations = 5000, minCycles = 0.5, maxCycles = 10, 
                       maxRatio = 5, penaltyWeight = 1, NoCosine=FALSE) {
  require(GenSA)
  ...
  # Convert a number of cycles (peaks) to an angular scale
  convertToAngle <- function(cycles) 2 * pi * cycles / n
  minSlope <- convertToAngle(minCycles / 2)
  maxSlope <- convertToAngle(maxCycles + 5)
  
  dataRange <- max(Y) - min(Y)
  maxYDeviation <- sqrt(sum(Y^2))  # maximum deviation when fit = 0
  
  if(!NoCosine){
    
    # Parameter vector order: b, level difference, B, slope1, slope2, slope3, bp1, bp2
    lowerBounds <- c(0, dataRange / 100, dataRange / 100, 0, 0, 0, 0, 0)
    upperBounds <- c(2*pi, dataRange, dataRange, rep(maxSlope, 3), n, n)
    
    # --- Model Function Definition ---
    # Combines a sine and cosine term for a flexible periodic model.
    modelFunction <- function(gx.arg, params) {
      b <- params[1]
      A <- params[3] + params[2]  # ensures primary amplitude A > B
      B <- params[3]
      A * sin(gx.arg + b) - B * cos(2 * gx.arg + 2 * b)
    }
    
    # --- Objective Function ---
    # Returns a function that computes the sum-of-squares error plus penalties.
    computeError <- function(x, y) {
      function(params) {
        # Check the ratios of consecutive slopes to control frequency jumps.
        slopeRatios <- c(params[4] / params[5], params[5] / params[6])
        if (any(is.na(slopeRatios)) || any(slopeRatios < 1 / maxRatio) || any(slopeRatios > maxRatio))
          return(Inf)
        
        # Use simple version g(x) = piecewise linear
        gx <- pwlinear(x, params[4:6], params[7] + c(0, params[8]))
        cycleCount <- gx[length(x)] / (2 * pi)
        
        # Penalize if the effective cycle count deviates from the expected range.
        penalty <- maxYDeviation * ((minCycles / cycleCount - 1) * (cycleCount < minCycles) +
                                      (cycleCount / maxCycles - 1) * (cycleCount > maxCycles))^2
        
        sum((modelFunction(gx, params) - y)^2) + penaltyWeight * penalty
      }
    }
    
    errorFunction <- computeError(x, Y)
    
    # --- Simulated Annealing Optimization ---
    optResult <- GenSA::GenSA(
      fn = errorFunction,
      lower = lowerBounds,
      upper = upperBounds,
      control = list(
    ...
    optParams <- optResult$par
    
    # --- Post-Processing of Optimized Parameters ---
    gx_final <- pwlinear(x, optParams[4:6], optParams[7] + c(0, optParams[8]))
    xi_final <- diff(gx_final)
    xi_final[n] <- xi_final[n - 1]  # Extend the last difference
    
    # Identify critical points (assumes FindXstar is defined elsewhere)
    xkstar_final <- sort(c(FindXstar(gx_final, optParams[1], pi/2),
                           FindXstar(gx_final, optParams[1], 3 * pi/2)))
    
    # Compute the fitted values (assumes function f is defined elsewhere)
    Y_fit <- f(gx_final, optParams[2], optParams[3], optParams[1])
    
    # --- Return Initialization Data ---
    initData <- list(
      pars = list(
        a0 = mean(diff(gx_final)),
        b0 = optParams[1],
        A0 = optParams[3] + optParams[2],
        B0 = optParams[3],
        sigma0 = sqrt(mean((Y - Y_fit)^2))
      ),
      gx = gx_final,
      xi = xi_final,
      xkstar = xkstar_final,
  ...
  }else{ # Simple model: No Cosine Term
    
    # Parameter vector order: b, A, slope1, slope2, slope3, bp1, bp2
    lowerBounds <- c(0, dataRange / 100, 0, 0, 0, 0, 0)
    upperBounds <- c(2*pi, dataRange, rep(maxSlope, 3), n, n)
    ...
    computeError <- function(x, y) {
      function(params) {
        # Check the ratios of consecutive slopes to control frequency jumps.
        slopeRatios <- c(params[3] / params[4], params[4] / params[5])
        if (any(is.na(slopeRatios)) || any(slopeRatios < 1 / maxRatio) || any(slopeRatios > maxRatio))
          return(Inf)
        
        # Use simple version g(x) = piecewise linear
        gx <- pwlinear(x, params[3:5], params[6] + c(0, params[7]))
        cycleCount <- gx[length(x)] / (2 * pi)
    ...
    # --- Post-Processing of Optimized Parameters ---
    gx_final <- pwlinear(x, optParams[3:5], optParams[6] + c(0, optParams[7]))
    xi_final <- diff(gx_final)
    xi_final[n] <- xi_final[n - 1]  # Extend the last difference
    ...
    # Compute the fitted values (assumes function f is defined elsewhere)
    Y_fit <- f(gx_final, optParams[2], 0, optParams[1])
